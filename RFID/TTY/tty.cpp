...
//const char dev_com[MAX_COM_NUM][20] = {"/dev/ttyS1\0", "/dev/ttyS2\0"};
//const char dev_com[MAX_COM_NUM][13] = {"/dev/ttySAC1", "/dev/ttySAC2"};
//#define COM1   "/dev/ttyS3"
...
int tty_init(const char* _name, int _baud)
{
    ...
    else qDebug("%s open successful!", _name);
...
{
   // tty[id].name = (char*)malloc(strlen(dev_com[id]));
//    memcpy(tty[id].name, dev_com[id], sizeof(dev_com[id]));
...
{
    QString str;
    HexToQString(str, tty[id].txbuf, tty[id].txlen);
    qDebug()<<"UART_TX:"<<str;
}
void PrintUartRxBuf(int id)
{
    QString str;
    HexToQString(str, tty[id].rxbuf, tty[id].rxlen);
    qDebug()<<"UART_RX"<<str;
}
